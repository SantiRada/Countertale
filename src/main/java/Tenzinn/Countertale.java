...
public class Countertale extends JavaPlugin {
    ...
    protected void setup() {
        ...
        getCommandRegistry().registerCommand(new ForceStartCommand("forcestart", "Force start game without fill players", this));
    }
    ...
    protected void start() {
        ...
        @SuppressWarnings("unchecked")
        ScheduledFuture<Void> matchTaskVoid = (ScheduledFuture<Void>) matchCheckTask;
        getTaskRegistry().registerTask(matchTaskVoid);

        getLogger().at(Level.INFO).log("Countertale plugin iniciado correctamente");
    }

    // ==================== MÉTODOS DE HUD (Ya existentes) ====================
    private void updateAllHuds() { activeHuds.values().forEach(ServerStatusHud::updateStats); }
    public void registerHud(String playerId, ServerStatusHud hud) { activeHuds.put(playerId, hud); }
    public void unregisterHud(String playerId) { activeHuds.remove(playerId); }
    ...
    private void checkAndStartFullMatches() {
        ...
        for (GameMatch match : fullMatches) { startMatch(match); }
    }
