...
public class PartyObject {
    ...
    public void AddPlayer(PlayerRef playerRef) {
        ...
        if (players.size() >= 5) {
            playerRef.sendMessage(Message.raw("The group is full (maximum 5 players).").color(Color.red));
            return;
        }

        players.add(playerRef);
        SendMessageToAllPlayers(playerRef.getUsername() + " has joined the group.");
