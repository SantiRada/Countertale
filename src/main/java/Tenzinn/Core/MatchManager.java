...
public class MatchManager implements InstancePool.MatchManagerInstanceCounter {
    ...
    public GameMatch addPlayerToQueue(PlayerRef playerRef, String mode) {
        ...
        if (availableMatch.isPresent()) {
            ...
            InstanceManager pooledInstance = instancePool.take(mode);
    ...
    public boolean removePlayerFromMatch(PlayerRef playerRef) {
        ...
        if (match.getPlayers().isEmpty()) {
            CompletableFuture.delayedExecutor(2, TimeUnit.SECONDS).execute(() -> {
                ...
                instancePool.refill(RefactorTool.getModeForPlayer(playerRef) == MapListeners.SpawnMode.DM ? "dm" : "fvf");
            });
        }
